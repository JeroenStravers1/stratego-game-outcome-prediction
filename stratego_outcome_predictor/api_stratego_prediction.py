...
def get_prediction(piece_positions, unmoved_pieces, unrevealed_pieces, source, target):
    ...
    dfc = calculate_features(piece_positions, unmoved_pieces, unrevealed_pieces, source, target)
    for key, element in dfc.extracted_features.iteritems():
...
def check_for_source_and_target_in_raw_payload(raw_payload, parsed_payload):
    ...
    if source != "":
        ...
        print "no source/target in json"
...
NSR = 0
# TODO: genereer lelijke html dynamisch en verstuur dat naar je browser. 
...
def say_hi(obj_response):
    ...
    reqs = get_request_number()
    #obj_response.alert('Hi there Jeroen!' + str(reqs))
    obj_response.html('#main_div', '<p>HALLOOOOOO</p>' + str(reqs))
